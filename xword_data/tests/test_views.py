...
class TestDrillView(TestCase):
    ...
    def test_drill_post_incorrect(self):
        ...
        data = {"clue_id": clue.id, "answer": clue.entry.entry_text + "wrong"}
        response = self.client.post(self.url, data=data)
        self.assertEqual(200, response.status_code)
        self.assertContains(response, "incorrect_answer")

    def test_drill_post_correct(self):
        clue = Clue.objects.order_by("?").first()
        data = {"clue_id": clue.id, "answer": clue.entry.entry_text.lower()}
        response = self.client.post(self.url, data=data)
    ...
    def test_drill_messaging(self):
        ...
        data = {"clue_id": clue.id, "answer": clue.entry.entry_text}
